...
export class AuthenticationService {
  ...
  public registerUser(param: User): Observable<User> {
    ...
    console.log(`Register user at ${endpoint}, param = ${param}`);
    return this.http.post<any>(endpoint, param).pipe(
      map((val) => {
  ...
  public login(email: String, password: String): Observable<User> {
    ...
    return this.http
      .post<any>(endpoint, { email, password }, { headers: this.headers })
      .pipe(
        map((val) => {
  ...
  public handleError(error: HttpErrorResponse) {
    ...
    if (error.error instanceof ErrorEvent) {
      ...
      errorMsg = `Backend returned code ${error.status} - ${error.statusText}. (Could not contact server)`;
    }
